	...
	xmlhttp.onreadystatechange=function()
	{
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			...
			for (i=0;i<items.length;i++)
			{
				...
				//xml_content += '<div id="date_' + i + '" class="col s2 center" style="clear:both;padding-top:5px;"><i class="grey-text text-darken-1 small mdi-notification-event-note"></i><div class="grey-text" style="margin-top:-5px;font-size:70%;">' + items[i].getElementsByTagName("pubDate")[0].textContent.substring(5,11) + '</div></div>';
				...
				xml_content += '<li>';
				xml_content += '<div class="collapsible-header"><i class="material-icons grey-text text-darken-1 small">event</i>' + items[i].getElementsByTagName("pubDate")[0].textContent.substring(5,11) + '-' + items[i].getElementsByTagName("title")[0].textContent + '</div>';
				xml_content += '<div class="collapsible-body">' + items[i].getElementsByTagName("description")[0].textContent + '</div>';
				xml_content += '</li>';
			}
