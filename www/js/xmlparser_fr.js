	...
	xmlhttp.onreadystatechange=function()
	{
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			...
			for (i=0;i<items.length;i++)
			{
				...
				xml_content += '<div id="title_' + i + '" class="col s10 item_title"><h5 style="padding-top:2px;font-size:110%"><a href="#a_' + i + '" onclick="read('+ i +')" style="color:#212121;">' + items[i].getElementsByTagName("title")[0].textContent + '</a></h5><br /></div>';
				xml_content += '<div id="' + i + '" class="col s12 item_description">';
				xml_content += items[i].getElementsByTagName("description")[0].textContent
				xml_content += '<a href="'+ items[i].getElementsByTagName("link")[0].textContent + '" class="waves-effect waves-light btn light-blue darken-3">suite...</a></div>';
				xml_content += '</div>';
			}
